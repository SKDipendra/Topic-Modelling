...
def makeGraph(topics,doc_no ):
    ...
    print(topics)
    print(doc_no)
    df = pd.DataFrame({'topics': topics, 'Number of Editorials': doc_no})
    #df = df.sort_values('doc_no', inplace=True)
    #topics = df.topics.values
    #doc_no = df.doc_no.values

    df = df.sort_values('Number of Editorials')

    fig, ax = plt.subplots()
    df.plot(kind='barh', y='Number of Editorials', x='topics', ax=ax)
...
def main():
    ...
    # Applying model
    ldamodel = gensim.models.ldamodel.LdaModel(corpus, num_topics=27, random_state=2, id2word=dictionary, passes=15)
    topics = ldamodel.print_topics(num_topics=27, num_words=3)
    list_topics_graph = []
    for topic in topics:
        #print(topic[1])
    ...
    #with open('topic_keywords.txt', 'w') as file:
        #for each in list_topics_graph:
            #file.write(each)
